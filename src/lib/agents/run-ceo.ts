...
export async function runCeoReview(): Promise<{ output: AgentOutput; dashboard: CeoDashboardData }> {
  ...
  const output = buildFallbackCeoOutput({
    metrics,
  ...
  const priorities = output.recommended_actions
  ...
  const nextActions = output.recommended_actions.map((action) => action.title).slice(0, 3);
  ...
  const refreshedLogs = await getRecentAgentLogs(10);
  const refreshedTasks = await getOpenAgentTasks(20);
